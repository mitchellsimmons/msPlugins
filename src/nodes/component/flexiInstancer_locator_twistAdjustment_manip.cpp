...
FlexiInstancer_TwistAdjustmentManip::FlexiInstancer_TwistAdjustmentManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_physicalIndex{ 0 },
...
FlexiInstancer_TwistAdjustmentManip::~FlexiInstancer_TwistAdjustmentManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiInstancer_TwistAdjustmentManip::physicalIndex = 0;
const MDistance FlexiInstancer_TwistAdjustmentManip::m_offsetHandleRadius{ 1.2, MDistance::kCentimeters };
const double FlexiInstancer_TwistAdjustmentManip::m_offsetHandleDragIncrement = 0.001;

// ------ MPxNode------

void FlexiInstancer_TwistAdjustmentManip::postConstructor()
{
...
MStatus FlexiInstancer_TwistAdjustmentManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiInstancer_TwistAdjustmentManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiInstancer_TwistAdjustmentManip::preDrawUI(const M3dView& view)
{
...
void FlexiInstancer_TwistAdjustmentManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiInstancer_TwistAdjustmentManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiInstancerTwistAdjustmentContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiInstancerTwistAdjustmentContext1";
			MGlobal::executeCommand(activePlugCmd);

			// Update the ramp UI if it exists and is visible (see MRS_FlexiInstancer_twistAdjustmentUIReplace.mel for implementation)
			...
			MString updateUICmd = "MRS_FlexiInstancer_twistAdjustmentUIReplace " + getCompoundPlugPartialName();
...
MStatus FlexiInstancer_TwistAdjustmentManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiInstancer_TwistAdjustmentManip::doRelease(M3dView& view)
{
...
MStatus FlexiInstancer_TwistAdjustmentManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiInstancer_TwistAdjustmentManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;

	MPlug compoundArrayPlug{ m_locatorPath.node(), FlexiInstancer::twistAdjustmentCompoundAttr };
...
MStatus FlexiInstancer_TwistAdjustmentManip::getRampCompoundPlug(MPlug& rampPlug)
{
	MStatus status;
	...
	if (status)
		rampPlug = compoundPlug.child(FlexiInstancer::twistAdjustmentRampAttr, &status);
...
MStatus FlexiInstancer_TwistAdjustmentManip::getTwistPlug(MPlug& twistPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		twistPlug = compoundPlug.child(FlexiInstancer::twistAdjustmentValueAttr, &status);
...
MStatus FlexiInstancer_TwistAdjustmentManip::getOffsetPlug(MPlug& offsetPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		offsetPlug = compoundPlug.child(FlexiInstancer::twistAdjustmentOffsetAttr, &status);
...
MString FlexiInstancer_TwistAdjustmentManip::getCompoundPlugPartialName()
{
...
bool FlexiInstancer_TwistAdjustmentManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiInstancer_TwistAdjustmentManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiInstancer_TwistAdjustmentManip::isActive()
{
	return m_isActive;
}
...
MPoint FlexiInstancer_TwistAdjustmentManip::sampleCurve(double param)
{
	MPoint pPosition;
...
MPoint FlexiInstancer_TwistAdjustmentManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiInstancer_TwistAdjustmentManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiInstancer_TwistAdjustmentManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiInstancer_TwistAdjustmentManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiInstancer_TwistAdjustmentManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiInstancer_TwistAdjustmentManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiInstancer_TwistAdjustmentManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane,
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiInstancer_TwistAdjustmentManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiInstancer_TwistAdjustmentManip::updateOffsetHandle()
{
...
MStatus FlexiInstancer_TwistAdjustmentManip::updateOffsetHandlePlug(double offset)
{
	MStatus status;
...
FlexiInstancer_TwistAdjustmentManipContainer::FlexiInstancer_TwistAdjustmentManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiInstancer_TwistAdjustmentManipContainer::~FlexiInstancer_TwistAdjustmentManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiInstancer_TwistAdjustmentManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiInstancer_TwistAdjustmentManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiInstancer_TwistAdjustmentManip::kTypeName);
	MString manipName("FlexiInstancerTwistAdjustmentManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiInstancer_TwistAdjustmentManip*)proxyManip;

	return status;
}

MStatus FlexiInstancer_TwistAdjustmentManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiInstancer_TwistAdjustmentContext::FlexiInstancer_TwistAdjustmentContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiInstancer Twist Adjustment Tool");
	setTitleString(titleStr);
}

FlexiInstancer_TwistAdjustmentContext::~FlexiInstancer_TwistAdjustmentContext() {}
...
void FlexiInstancer_TwistAdjustmentContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiInstancer_TwistAdjustmentContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiInstancer_TwistAdjustmentContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiInstancer_TwistAdjustmentContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiInstancer_TwistAdjustmentContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the offset for each twist adjustment falloff. Right click for more options.");
...
MStatus FlexiInstancer_TwistAdjustmentContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiInstancer  :  Twist Adjustment Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiInstancer  :  Twist Adjustment Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiInstancer_TwistAdjustmentContext::getHoverManipPlug()
{
	for (FlexiInstancer_TwistAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiInstancer_TwistAdjustmentContext::getActiveManipPlug()
{
	for (FlexiInstancer_TwistAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiInstancer_TwistAdjustmentContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiInstancer_TwistAdjustmentContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiInstancer_TwistAdjustmentContext* context = (FlexiInstancer_TwistAdjustmentContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiInstancer::kTypeId)
		{
			...
			{
				FlexiInstancer_TwistAdjustmentManip::physicalIndex = i;
				MString manipName(FlexiInstancer_TwistAdjustmentManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiInstancer_TwistAdjustmentManipContainer* manipContainer = (FlexiInstancer_TwistAdjustmentManipContainer*)FlexiInstancer_TwistAdjustmentManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiInstancer_TwistAdjustmentContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiInstancer_TwistAdjustmentContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
		if (plug == FlexiInstancer::outputLocalParticleArrayAttr || plug == FlexiInstancer::outputWorldParticleArrayAttr)
		{
			...
			FlexiInstancer_TwistAdjustmentContext* context = (FlexiInstancer_TwistAdjustmentContext*)data;
			for (FlexiInstancer_TwistAdjustmentManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiInstancerTwistAdjustmentContext command is used to generate a custom context which can then be used to modify twist adjustments
	- A manipulator is created for each twistAdjustment element plug, on each selected FlexiInstancer node assigned to the active context
	- This command allows the user to query certain states relating to those manipulators

	MEL Command
	-----------
	FlexiInstancerTwistAdjustmentContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiInstancerTwistAdjustmentContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiInstancer_TwistAdjustmentContextCommand::FlexiInstancer_TwistAdjustmentContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiInstancer_TwistAdjustmentContextCommand::~FlexiInstancer_TwistAdjustmentContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiInstancer_TwistAdjustmentContextCommand::kHoverFlag = "-hmp";
const char* FlexiInstancer_TwistAdjustmentContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiInstancer_TwistAdjustmentContextCommand::kActiveFlag = "-amp";
const char* FlexiInstancer_TwistAdjustmentContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiInstancer_TwistAdjustmentContextCommand::makeObj()
{
	m_context = new FlexiInstancer_TwistAdjustmentContext;
	return m_context;
}

MStatus FlexiInstancer_TwistAdjustmentContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiInstancer_TwistAdjustmentContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiInstancer_TwistAdjustmentContextCommand::appendSyntax()
{
	MStatus status;
