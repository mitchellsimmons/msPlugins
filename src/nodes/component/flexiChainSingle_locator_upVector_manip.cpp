...
/*	Description
	-----------
	The below setup is designed to provide a way to manually override the up-vector of a FlexiChainSingle node
	...
	Interface
	---------
	Plugin registration instantiates a single instance of the FlexiChainSingle_UpVectorContext (MPxSelectionContext) which will be named "FlexiChainSingleUpVectorContext1"
	A custom marking menu is also registered as a callback, such that right clicking a FlexiChainSingle node will bring up a menu allowing the user to activate the context
	Once activated, the context retrieves data from attributes on each FlexiChainSingle node that is registered to the active context
	Each node provides data to two manipulators, a start and end FlexiChainSingle_UpVectorManip (MPxManipulatorNode)
	Both manipulators are encapsulated by a FlexiChainSingle_UpVectorManipContainer (MPxManipContainer) because of a selection issue detailed below
...
FlexiChainSingle_UpVectorManip::FlexiChainSingle_UpVectorManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_directionIndex{ 0 },
	m_activeName{ 0 },
	m_handleName{ 0 },
...
FlexiChainSingle_UpVectorManip::~FlexiChainSingle_UpVectorManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiChainSingle_UpVectorManip::positionIndex = 0;
const MDistance FlexiChainSingle_UpVectorManip::m_handleRadius{ 1.5, MDistance::kCentimeters };

const float FlexiChainSingle_UpVectorManip::m_tickPoints[][4]
{
...
const float FlexiChainSingle_UpVectorManip::m_crossPoints[][4]
{
...
void FlexiChainSingle_UpVectorManip::postConstructor()
{
...
MStatus FlexiChainSingle_UpVectorManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiChainSingle_UpVectorManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiChainSingle_UpVectorManip::preDrawUI(const M3dView& view)
{
...
void FlexiChainSingle_UpVectorManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiChainSingle_UpVectorManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiChainSingleUpVectorContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiChainSingleUpVectorContext1";
...
MStatus FlexiChainSingle_UpVectorManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiChainSingle_UpVectorManip::doRelease(M3dView& view)
{
...
MStatus FlexiChainSingle_UpVectorManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiChainSingle_UpVectorManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;
	
	if (m_positionIndex == 0)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiChainSingle::normalUpVectorOverrideCompoundAttr };
	else if (m_positionIndex == 1)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiChainSingle::counterTwistUpVectorOverrideCompoundAttr };
...
MStatus FlexiChainSingle_UpVectorManip::getDirectionPlug(MPlug& directionPlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			directionPlug = compoundPlug.child(FlexiChainSingle::normalUpVectorOverrideAttr, &status);
		else if (m_positionIndex == 1)
			directionPlug = compoundPlug.child(FlexiChainSingle::counterTwistUpVectorOverrideAttr, &status);
	}
...
MStatus FlexiChainSingle_UpVectorManip::getStatePlug(MPlug& statePlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			statePlug = compoundPlug.child(FlexiChainSingle::normalUpVectorOverrideStateAttr, &status);
		else if (m_positionIndex == 1)
			statePlug = compoundPlug.child(FlexiChainSingle::counterTwistUpVectorOverrideStateAttr, &status);
	}
...
MString FlexiChainSingle_UpVectorManip::getCompoundPlugPartialName()
{
...
bool FlexiChainSingle_UpVectorManip::isEnabled()
{
	MStatus status;
...
bool FlexiChainSingle_UpVectorManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiChainSingle_UpVectorManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiChainSingle_UpVectorManip::isActive()
{
	return m_isActive;
}

// Legacy implementation
MPoint FlexiChainSingle_UpVectorManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiChainSingle_UpVectorManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainSingle_UpVectorManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainSingle_UpVectorManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainSingle_UpVectorManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainSingle_UpVectorManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiChainSingle_UpVectorManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane, 
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiChainSingle_UpVectorManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiChainSingle_UpVectorManip::updateManipState()
{
	m_isEnabled = isEnabled();
}

void FlexiChainSingle_UpVectorManip::updateManipPoints()
{
...
MStatus FlexiChainSingle_UpVectorManip::updatePlugDirection(MVector& vDirection)
{
	MStatus status;
...
FlexiChainSingle_UpVectorManipContainer::FlexiChainSingle_UpVectorManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiChainSingle_UpVectorManipContainer::~FlexiChainSingle_UpVectorManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiChainSingle_UpVectorManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiChainSingle_UpVectorManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiChainSingle_UpVectorManip::kTypeName);
	MString manipName("FlexiChainSingleManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiChainSingle_UpVectorManip*)proxyManip;

	return status;
}

MStatus FlexiChainSingle_UpVectorManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiChainSingle_UpVectorContext::FlexiChainSingle_UpVectorContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiChainSingle Up-Vector Tool");
	setTitleString(titleStr);
}

FlexiChainSingle_UpVectorContext::~FlexiChainSingle_UpVectorContext() {}
...
void FlexiChainSingle_UpVectorContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiChainSingle_UpVectorContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiChainSingle_UpVectorContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiChainSingle_UpVectorContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiChainSingle_UpVectorContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the up-vector to be used at each end. Right click for more options.");
...
MStatus FlexiChainSingle_UpVectorContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiChainSingle  :  Up-Vector Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiChainSingle  :  Up-Vector Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiChainSingle_UpVectorContext::getHoverManipPlug()
{
	for (FlexiChainSingle_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiChainSingle_UpVectorContext::getActiveManipPlug()
{
	for (FlexiChainSingle_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiChainSingle_UpVectorContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiChainSingle_UpVectorContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiChainSingle_UpVectorContext* context = (FlexiChainSingle_UpVectorContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiChainSingle::kTypeId)
		{
			...
			{
				FlexiChainSingle_UpVectorManip::positionIndex = i;
				MString manipName(FlexiChainSingle_UpVectorManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiChainSingle_UpVectorManipContainer* manipContainer = (FlexiChainSingle_UpVectorManipContainer*)FlexiChainSingle_UpVectorManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiChainSingle_UpVectorContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiChainSingle_UpVectorContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
			...
			plug == FlexiChainSingle::outputWorldPositionsAttr || plug == FlexiChainSingle::outputWorldOrientationsAttr)
		{
			...
			FlexiChainSingle_UpVectorContext* context = (FlexiChainSingle_UpVectorContext*)data;
			for (FlexiChainSingle_UpVectorManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiChainSingleUpVectorContext command is used to generate a custom context which can then be used to set manual overrides for the up-vectors
	...
	MEL Command
	-----------
	FlexiChainSingleUpVectorContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiChainSingleUpVectorContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiChainSingle_UpVectorContextCommand::FlexiChainSingle_UpVectorContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiChainSingle_UpVectorContextCommand::~FlexiChainSingle_UpVectorContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiChainSingle_UpVectorContextCommand::kHoverFlag = "-hmp";
const char* FlexiChainSingle_UpVectorContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiChainSingle_UpVectorContextCommand::kActiveFlag = "-amp";
const char* FlexiChainSingle_UpVectorContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiChainSingle_UpVectorContextCommand::makeObj()
{
	m_context = new FlexiChainSingle_UpVectorContext;
	return m_context;
}

MStatus FlexiChainSingle_UpVectorContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiChainSingle_UpVectorContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiChainSingle_UpVectorContextCommand::appendSyntax()
{
	MStatus status;
