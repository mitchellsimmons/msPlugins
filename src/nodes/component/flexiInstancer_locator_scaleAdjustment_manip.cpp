...
FlexiInstancer_ScaleAdjustmentManip::FlexiInstancer_ScaleAdjustmentManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_physicalIndex{ 0 },
...
FlexiInstancer_ScaleAdjustmentManip::~FlexiInstancer_ScaleAdjustmentManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiInstancer_ScaleAdjustmentManip::physicalIndex = 0;
const MDistance FlexiInstancer_ScaleAdjustmentManip::m_offsetHandleRadius{ 1.2, MDistance::kCentimeters };
const double FlexiInstancer_ScaleAdjustmentManip::m_offsetHandleDragIncrement = 0.001;

// ------ MPxNode------

void FlexiInstancer_ScaleAdjustmentManip::postConstructor()
{
...
MStatus FlexiInstancer_ScaleAdjustmentManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiInstancer_ScaleAdjustmentManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiInstancer_ScaleAdjustmentManip::preDrawUI(const M3dView& view)
{
...
void FlexiInstancer_ScaleAdjustmentManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiInstancer_ScaleAdjustmentManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiInstancerScaleAdjustmentContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiInstancerScaleAdjustmentContext1";
			MGlobal::executeCommand(activePlugCmd);

			// Update the ramp UI if it exists and is visible (see MRS_FlexiInstancer_scaleAdjustmentUIReplace.mel for implementation)
			...
			MString updateUICmd = "MRS_FlexiInstancer_scaleAdjustmentUIReplace " + getCompoundPlugPartialName();
...
MStatus FlexiInstancer_ScaleAdjustmentManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiInstancer_ScaleAdjustmentManip::doRelease(M3dView& view)
{
...
MStatus FlexiInstancer_ScaleAdjustmentManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiInstancer_ScaleAdjustmentManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;

	MPlug compoundArrayPlug{ m_locatorPath.node(), FlexiInstancer::scaleAdjustmentCompoundAttr };
...
MStatus FlexiInstancer_ScaleAdjustmentManip::getRampCompoundPlug(MPlug& rampPlug)
{
	MStatus status;
	...
	if (status)
		rampPlug = compoundPlug.child(FlexiInstancer::scaleAdjustmentRampAttr, &status);
...
MStatus FlexiInstancer_ScaleAdjustmentManip::getScalePlug(MPlug& scalePlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		scalePlug = compoundPlug.child(FlexiInstancer::scaleAdjustmentValueAttr, &status);
...
MStatus FlexiInstancer_ScaleAdjustmentManip::getOffsetPlug(MPlug& offsetPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		offsetPlug = compoundPlug.child(FlexiInstancer::scaleAdjustmentOffsetAttr, &status);
...
MString FlexiInstancer_ScaleAdjustmentManip::getCompoundPlugPartialName()
{
...
bool FlexiInstancer_ScaleAdjustmentManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiInstancer_ScaleAdjustmentManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiInstancer_ScaleAdjustmentManip::isActive()
{
	return m_isActive;
}
...
MPoint FlexiInstancer_ScaleAdjustmentManip::sampleCurve(double param)
{
	MPoint pPosition;
...
MPoint FlexiInstancer_ScaleAdjustmentManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiInstancer_ScaleAdjustmentManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiInstancer_ScaleAdjustmentManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiInstancer_ScaleAdjustmentManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiInstancer_ScaleAdjustmentManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiInstancer_ScaleAdjustmentManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiInstancer_ScaleAdjustmentManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane,
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiInstancer_ScaleAdjustmentManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiInstancer_ScaleAdjustmentManip::updateOffsetHandle()
{
...
MStatus FlexiInstancer_ScaleAdjustmentManip::updateOffsetHandlePlug(double offset)
{
	MStatus status;
...
FlexiInstancer_ScaleAdjustmentManipContainer::FlexiInstancer_ScaleAdjustmentManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiInstancer_ScaleAdjustmentManipContainer::~FlexiInstancer_ScaleAdjustmentManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiInstancer_ScaleAdjustmentManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiInstancer_ScaleAdjustmentManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiInstancer_ScaleAdjustmentManip::kTypeName);
	MString manipName("FlexiInstancerScaleAdjustmentManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiInstancer_ScaleAdjustmentManip*)proxyManip;

	return status;
}

MStatus FlexiInstancer_ScaleAdjustmentManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiInstancer_ScaleAdjustmentContext::FlexiInstancer_ScaleAdjustmentContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiInstancer Scale Adjustment Tool");
	setTitleString(titleStr);
}

FlexiInstancer_ScaleAdjustmentContext::~FlexiInstancer_ScaleAdjustmentContext() {}
...
void FlexiInstancer_ScaleAdjustmentContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiInstancer_ScaleAdjustmentContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiInstancer_ScaleAdjustmentContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiInstancer_ScaleAdjustmentContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiInstancer_ScaleAdjustmentContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the offset for each scale adjustment falloff. Right click for more options.");
...
MStatus FlexiInstancer_ScaleAdjustmentContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiInstancer  :  Scale Adjustment Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiInstancer  :  Scale Adjustment Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiInstancer_ScaleAdjustmentContext::getHoverManipPlug()
{
	for (FlexiInstancer_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiInstancer_ScaleAdjustmentContext::getActiveManipPlug()
{
	for (FlexiInstancer_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiInstancer_ScaleAdjustmentContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiInstancer_ScaleAdjustmentContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiInstancer_ScaleAdjustmentContext* context = (FlexiInstancer_ScaleAdjustmentContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiInstancer::kTypeId)
		{
			...
			{
				FlexiInstancer_ScaleAdjustmentManip::physicalIndex = i;
				MString manipName(FlexiInstancer_ScaleAdjustmentManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiInstancer_ScaleAdjustmentManipContainer* manipContainer = (FlexiInstancer_ScaleAdjustmentManipContainer*)FlexiInstancer_ScaleAdjustmentManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiInstancer_ScaleAdjustmentContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiInstancer_ScaleAdjustmentContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
		if (plug == FlexiInstancer::outputLocalParticleArrayAttr || plug == FlexiInstancer::outputWorldParticleArrayAttr)
		{
			...
			FlexiInstancer_ScaleAdjustmentContext* context = (FlexiInstancer_ScaleAdjustmentContext*)data;
			for (FlexiInstancer_ScaleAdjustmentManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiInstancerScaleAdjustmentContext command is used to generate a custom context which can then be used to modify scale adjustments
	- A manipulator is created for each scaleAdjustment element plug, on each selected FlexiInstancer node assigned to the active context
	- This command allows the user to query certain states relating to those manipulators

	MEL Command
	-----------
	FlexiInstancerScaleAdjustmentContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiInstancerScaleAdjustmentContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiInstancer_ScaleAdjustmentContextCommand::FlexiInstancer_ScaleAdjustmentContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiInstancer_ScaleAdjustmentContextCommand::~FlexiInstancer_ScaleAdjustmentContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiInstancer_ScaleAdjustmentContextCommand::kHoverFlag = "-hmp";
const char* FlexiInstancer_ScaleAdjustmentContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiInstancer_ScaleAdjustmentContextCommand::kActiveFlag = "-amp";
const char* FlexiInstancer_ScaleAdjustmentContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiInstancer_ScaleAdjustmentContextCommand::makeObj()
{
	m_context = new FlexiInstancer_ScaleAdjustmentContext;
	return m_context;
}

MStatus FlexiInstancer_ScaleAdjustmentContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiInstancer_ScaleAdjustmentContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiInstancer_ScaleAdjustmentContextCommand::appendSyntax()
{
	MStatus status;
