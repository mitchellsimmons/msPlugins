...
/*	Description
	-----------
	The below setup is designed to provide a way to manually override the up-vector of a FlexiChainDouble node
	...
	Interface
	---------
	Plugin registration instantiates a single instance of the FlexiChainDouble_UpVectorContext (MPxSelectionContext) which will be named "FlexiChainDoubleUpVectorContext1"
	A custom marking menu is also registered as a callback, such that right clicking a FlexiChainDouble node will bring up a menu allowing the user to activate the context
	Once activated, the context retrieves data from attributes on each FlexiChainDouble node that is registered to the active context
	Each node provides data to two manipulators, a start and end FlexiChainDouble_UpVectorManip (MPxManipulatorNode)
	Both manipulators are encapsulated by a FlexiChainDouble_UpVectorManipContainer (MPxManipContainer) because of a selection issue detailed below
...
FlexiChainDouble_UpVectorManip::FlexiChainDouble_UpVectorManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_directionIndex{ 0 },
	m_activeName{ 0 },
	m_handleName{ 0 },
...
FlexiChainDouble_UpVectorManip::~FlexiChainDouble_UpVectorManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiChainDouble_UpVectorManip::positionIndex = 0;
const MDistance FlexiChainDouble_UpVectorManip::m_handleRadius{ 1.5, MDistance::kCentimeters };

const float FlexiChainDouble_UpVectorManip::m_tickPoints[][4]
{
...
const float FlexiChainDouble_UpVectorManip::m_crossPoints[][4]
{
...
void FlexiChainDouble_UpVectorManip::postConstructor()
{
...
MStatus FlexiChainDouble_UpVectorManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiChainDouble_UpVectorManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiChainDouble_UpVectorManip::preDrawUI(const M3dView& view)
{
...
void FlexiChainDouble_UpVectorManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiChainDouble_UpVectorManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiChainDoubleUpVectorContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiChainDoubleUpVectorContext1";
...
MStatus FlexiChainDouble_UpVectorManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiChainDouble_UpVectorManip::doRelease(M3dView& view)
{
...
MStatus FlexiChainDouble_UpVectorManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiChainDouble_UpVectorManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;
	
	if (m_positionIndex == 0)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiChainDouble::normalUpVectorOverrideCompoundAttr };
	else if (m_positionIndex == 1)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiChainDouble::counterTwistUpVectorOverrideCompoundAttr };
...
MStatus FlexiChainDouble_UpVectorManip::getDirectionPlug(MPlug& directionPlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			directionPlug = compoundPlug.child(FlexiChainDouble::normalUpVectorOverrideAttr, &status);
		else if (m_positionIndex == 1)
			directionPlug = compoundPlug.child(FlexiChainDouble::counterTwistUpVectorOverrideAttr, &status);
	}
...
MStatus FlexiChainDouble_UpVectorManip::getStatePlug(MPlug& statePlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			statePlug = compoundPlug.child(FlexiChainDouble::normalUpVectorOverrideStateAttr, &status);
		else if (m_positionIndex == 1)
			statePlug = compoundPlug.child(FlexiChainDouble::counterTwistUpVectorOverrideStateAttr, &status);
	}
...
MString FlexiChainDouble_UpVectorManip::getCompoundPlugPartialName()
{
...
bool FlexiChainDouble_UpVectorManip::isEnabled()
{
	MStatus status;
...
bool FlexiChainDouble_UpVectorManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiChainDouble_UpVectorManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiChainDouble_UpVectorManip::isActive()
{
	return m_isActive;
}

// Legacy implementation
MPoint FlexiChainDouble_UpVectorManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiChainDouble_UpVectorManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainDouble_UpVectorManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainDouble_UpVectorManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainDouble_UpVectorManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainDouble_UpVectorManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiChainDouble_UpVectorManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane, 
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiChainDouble_UpVectorManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiChainDouble_UpVectorManip::updateManipState()
{
	m_isEnabled = isEnabled();
}

void FlexiChainDouble_UpVectorManip::updateManipPoints()
{
	const FlexiChainDouble::FlexiChainDouble_Data& curveData = m_locator->getCurveData();
	if (m_positionIndex == 0)
		m_pLineStart = curveData.points[0] * m_locatorPath.inclusiveMatrix();
	else
		m_pLineStart = curveData.points[curveData.sampleCount - 1] * m_locatorPath.inclusiveMatrix();
...
MStatus FlexiChainDouble_UpVectorManip::updatePlugDirection(MVector& vDirection)
{
	MStatus status;
...
FlexiChainDouble_UpVectorManipContainer::FlexiChainDouble_UpVectorManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiChainDouble_UpVectorManipContainer::~FlexiChainDouble_UpVectorManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiChainDouble_UpVectorManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiChainDouble_UpVectorManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiChainDouble_UpVectorManip::kTypeName);
	MString manipName("FlexiChainDoubleManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiChainDouble_UpVectorManip*)proxyManip;

	return status;
}

MStatus FlexiChainDouble_UpVectorManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiChainDouble_UpVectorContext::FlexiChainDouble_UpVectorContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiChainDouble Up-Vector Tool");
	setTitleString(titleStr);
}

FlexiChainDouble_UpVectorContext::~FlexiChainDouble_UpVectorContext() {}
...
void FlexiChainDouble_UpVectorContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiChainDouble_UpVectorContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiChainDouble_UpVectorContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiChainDouble_UpVectorContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiChainDouble_UpVectorContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the up-vector to be used at each end. Right click for more options.");
...
MStatus FlexiChainDouble_UpVectorContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiChainDouble  :  Up-Vector Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiChainDouble  :  Up-Vector Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiChainDouble_UpVectorContext::getHoverManipPlug()
{
	for (FlexiChainDouble_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiChainDouble_UpVectorContext::getActiveManipPlug()
{
	for (FlexiChainDouble_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiChainDouble_UpVectorContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiChainDouble_UpVectorContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiChainDouble_UpVectorContext* context = (FlexiChainDouble_UpVectorContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiChainDouble::kTypeId)
		{
			...
			{
				FlexiChainDouble_UpVectorManip::positionIndex = i;
				MString manipName(FlexiChainDouble_UpVectorManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiChainDouble_UpVectorManipContainer* manipContainer = (FlexiChainDouble_UpVectorManipContainer*)FlexiChainDouble_UpVectorManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiChainDouble_UpVectorContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiChainDouble_UpVectorContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
			...
			plug == FlexiChainDouble::outputWorldPositionsAttr || plug == FlexiChainDouble::outputWorldOrientationsAttr)
		{
			...
			FlexiChainDouble_UpVectorContext* context = (FlexiChainDouble_UpVectorContext*)data;
			for (FlexiChainDouble_UpVectorManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiChainDoubleUpVectorContext command is used to generate a custom context which can then be used to set manual overrides for the up-vectors
	...
	MEL Command
	-----------
	FlexiChainDoubleUpVectorContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiChainDoubleUpVectorContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiChainDouble_UpVectorContextCommand::FlexiChainDouble_UpVectorContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiChainDouble_UpVectorContextCommand::~FlexiChainDouble_UpVectorContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiChainDouble_UpVectorContextCommand::kHoverFlag = "-hmp";
const char* FlexiChainDouble_UpVectorContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiChainDouble_UpVectorContextCommand::kActiveFlag = "-amp";
const char* FlexiChainDouble_UpVectorContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiChainDouble_UpVectorContextCommand::makeObj()
{
	m_context = new FlexiChainDouble_UpVectorContext;
	return m_context;
}

MStatus FlexiChainDouble_UpVectorContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiChainDouble_UpVectorContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiChainDouble_UpVectorContextCommand::appendSyntax()
{
	MStatus status;
