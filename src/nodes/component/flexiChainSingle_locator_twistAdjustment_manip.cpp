...
FlexiChainSingle_TwistAdjustmentManip::FlexiChainSingle_TwistAdjustmentManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_physicalIndex{ 0 },
...
FlexiChainSingle_TwistAdjustmentManip::~FlexiChainSingle_TwistAdjustmentManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiChainSingle_TwistAdjustmentManip::physicalIndex = 0;
const MDistance FlexiChainSingle_TwistAdjustmentManip::m_offsetHandleRadius{ 1.2, MDistance::kCentimeters };
const double FlexiChainSingle_TwistAdjustmentManip::m_offsetHandleDragIncrement = 0.001;

// ------ MPxNode------

void FlexiChainSingle_TwistAdjustmentManip::postConstructor()
{
...
MStatus FlexiChainSingle_TwistAdjustmentManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiChainSingle_TwistAdjustmentManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiChainSingle_TwistAdjustmentManip::preDrawUI(const M3dView& view)
{
...
void FlexiChainSingle_TwistAdjustmentManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiChainSingle_TwistAdjustmentManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiChainSingleTwistAdjustmentContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiChainSingleTwistAdjustmentContext1";
			MGlobal::executeCommand(activePlugCmd);

			// Update the ramp UI if it exists and is visible (see MRS_FlexiChainSingle_twistAdjustmentUIReplace.mel for implementation)
			...
			MString updateUICmd = "MRS_FlexiChainSingle_twistAdjustmentUIReplace " + getCompoundPlugPartialName();
...
MStatus FlexiChainSingle_TwistAdjustmentManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiChainSingle_TwistAdjustmentManip::doRelease(M3dView& view)
{
...
MStatus FlexiChainSingle_TwistAdjustmentManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiChainSingle_TwistAdjustmentManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;

	MPlug compoundArrayPlug{ m_locatorPath.node(), FlexiChainSingle::twistAdjustmentCompoundAttr };
...
MStatus FlexiChainSingle_TwistAdjustmentManip::getRampCompoundPlug(MPlug& rampPlug)
{
	MStatus status;
	...
	if (status)
		rampPlug = compoundPlug.child(FlexiChainSingle::twistAdjustmentRampAttr, &status);
...
MStatus FlexiChainSingle_TwistAdjustmentManip::getTwistPlug(MPlug& twistPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		twistPlug = compoundPlug.child(FlexiChainSingle::twistAdjustmentValueAttr, &status);
...
MStatus FlexiChainSingle_TwistAdjustmentManip::getOffsetPlug(MPlug& offsetPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		offsetPlug = compoundPlug.child(FlexiChainSingle::twistAdjustmentOffsetAttr, &status);
...
MString FlexiChainSingle_TwistAdjustmentManip::getCompoundPlugPartialName()
{
...
bool FlexiChainSingle_TwistAdjustmentManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiChainSingle_TwistAdjustmentManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiChainSingle_TwistAdjustmentManip::isActive()
{
	return m_isActive;
}
...
MPoint FlexiChainSingle_TwistAdjustmentManip::sampleCurve(double param)
{
	MPoint pPosition;
...
MPoint FlexiChainSingle_TwistAdjustmentManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiChainSingle_TwistAdjustmentManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainSingle_TwistAdjustmentManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainSingle_TwistAdjustmentManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainSingle_TwistAdjustmentManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainSingle_TwistAdjustmentManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiChainSingle_TwistAdjustmentManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane,
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiChainSingle_TwistAdjustmentManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiChainSingle_TwistAdjustmentManip::updateOffsetHandle()
{
...
MStatus FlexiChainSingle_TwistAdjustmentManip::updateOffsetHandlePlug(double offset)
{
	MStatus status;
...
FlexiChainSingle_TwistAdjustmentManipContainer::FlexiChainSingle_TwistAdjustmentManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiChainSingle_TwistAdjustmentManipContainer::~FlexiChainSingle_TwistAdjustmentManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiChainSingle_TwistAdjustmentManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiChainSingle_TwistAdjustmentManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiChainSingle_TwistAdjustmentManip::kTypeName);
	MString manipName("FlexiChainSingleTwistAdjustmentManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiChainSingle_TwistAdjustmentManip*)proxyManip;

	return status;
}

MStatus FlexiChainSingle_TwistAdjustmentManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiChainSingle_TwistAdjustmentContext::FlexiChainSingle_TwistAdjustmentContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiChainSingle Twist Adjustment Tool");
	setTitleString(titleStr);
}

FlexiChainSingle_TwistAdjustmentContext::~FlexiChainSingle_TwistAdjustmentContext() {}
...
void FlexiChainSingle_TwistAdjustmentContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiChainSingle_TwistAdjustmentContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiChainSingle_TwistAdjustmentContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiChainSingle_TwistAdjustmentContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiChainSingle_TwistAdjustmentContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the offset for each twist adjustment falloff. Right click for more options.");
...
MStatus FlexiChainSingle_TwistAdjustmentContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiChainSingle  :  Twist Adjustment Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiChainSingle  :  Twist Adjustment Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiChainSingle_TwistAdjustmentContext::getHoverManipPlug()
{
	for (FlexiChainSingle_TwistAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiChainSingle_TwistAdjustmentContext::getActiveManipPlug()
{
	for (FlexiChainSingle_TwistAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiChainSingle_TwistAdjustmentContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiChainSingle_TwistAdjustmentContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiChainSingle_TwistAdjustmentContext* context = (FlexiChainSingle_TwistAdjustmentContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiChainSingle::kTypeId)
		{
			...
			{
				FlexiChainSingle_TwistAdjustmentManip::physicalIndex = i;
				MString manipName(FlexiChainSingle_TwistAdjustmentManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiChainSingle_TwistAdjustmentManipContainer* manipContainer = (FlexiChainSingle_TwistAdjustmentManipContainer*)FlexiChainSingle_TwistAdjustmentManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiChainSingle_TwistAdjustmentContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiChainSingle_TwistAdjustmentContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
			...
			plug == FlexiChainSingle::outputWorldPositionsAttr || plug == FlexiChainSingle::outputWorldOrientationsAttr)
		{
			...
			FlexiChainSingle_TwistAdjustmentContext* context = (FlexiChainSingle_TwistAdjustmentContext*)data;
			for (FlexiChainSingle_TwistAdjustmentManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiChainSingleTwistAdjustmentContext command is used to generate a custom context which can then be used to modify twist adjustments
	- A manipulator is created for each twistAdjustment element plug, on each selected FlexiChainSingle node assigned to the active context
	- This command allows the user to query certain states relating to those manipulators

	MEL Command
	-----------
	FlexiChainSingleTwistAdjustmentContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiChainSingleTwistAdjustmentContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiChainSingle_TwistAdjustmentContextCommand::FlexiChainSingle_TwistAdjustmentContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiChainSingle_TwistAdjustmentContextCommand::~FlexiChainSingle_TwistAdjustmentContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiChainSingle_TwistAdjustmentContextCommand::kHoverFlag = "-hmp";
const char* FlexiChainSingle_TwistAdjustmentContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiChainSingle_TwistAdjustmentContextCommand::kActiveFlag = "-amp";
const char* FlexiChainSingle_TwistAdjustmentContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiChainSingle_TwistAdjustmentContextCommand::makeObj()
{
	m_context = new FlexiChainSingle_TwistAdjustmentContext;
	return m_context;
}

MStatus FlexiChainSingle_TwistAdjustmentContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiChainSingle_TwistAdjustmentContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiChainSingle_TwistAdjustmentContextCommand::appendSyntax()
{
	MStatus status;
