...
FlexiChainSingle_ScaleAdjustmentManip::FlexiChainSingle_ScaleAdjustmentManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_physicalIndex{ 0 },
...
FlexiChainSingle_ScaleAdjustmentManip::~FlexiChainSingle_ScaleAdjustmentManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiChainSingle_ScaleAdjustmentManip::physicalIndex = 0;
const MDistance FlexiChainSingle_ScaleAdjustmentManip::m_offsetHandleRadius{ 1.2, MDistance::kCentimeters };
const double FlexiChainSingle_ScaleAdjustmentManip::m_offsetHandleDragIncrement = 0.001;

// ------ MPxNode------

void FlexiChainSingle_ScaleAdjustmentManip::postConstructor()
{
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::connectToDependNode(const MObject& node)
{
	MStatus status;

	MDagPath::getAPathTo(node, m_locatorPath);
	MFnDependencyNode fnDep{ node };
	m_locator = (FlexiChainSingle*)fnDep.userNode();
...
void FlexiChainSingle_ScaleAdjustmentManip::preDrawUI(const M3dView& view)
{
...
void FlexiChainSingle_ScaleAdjustmentManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiChainSingleScaleAdjustmentContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiChainSingleScaleAdjustmentContext1";
			MGlobal::executeCommand(activePlugCmd);

			// Update the ramp UI if it exists and is visible (see MRS_FlexiChainSingle_scaleAdjustmentUIReplace.mel for implementation)
			...
			MString updateUICmd = "MRS_FlexiChainSingle_scaleAdjustmentUIReplace " + getCompoundPlugPartialName();
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::doRelease(M3dView& view)
{
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;

	MPlug compoundArrayPlug{ m_locatorPath.node(), FlexiChainSingle::scaleAdjustmentCompoundAttr };
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::getRampCompoundPlug(MPlug& rampPlug)
{
	MStatus status;
	...
	if (status)
		rampPlug = compoundPlug.child(FlexiChainSingle::scaleAdjustmentRampAttr, &status);
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::getScalePlug(MPlug& scalePlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		scalePlug = compoundPlug.child(FlexiChainSingle::scaleAdjustmentValueAttr, &status);
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::getOffsetPlug(MPlug& offsetPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		offsetPlug = compoundPlug.child(FlexiChainSingle::scaleAdjustmentOffsetAttr, &status);
...
MString FlexiChainSingle_ScaleAdjustmentManip::getCompoundPlugPartialName()
{
...
bool FlexiChainSingle_ScaleAdjustmentManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiChainSingle_ScaleAdjustmentManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiChainSingle_ScaleAdjustmentManip::isActive()
{
	return m_isActive;
}
...
MPoint FlexiChainSingle_ScaleAdjustmentManip::sampleCurve(double param)
{
	MPoint pPosition;
	...
	if (param < 0.0)
	{
		MVector vLowerBoundTangent = curveData.controlPoints[1] - curveData.controlPoints[0];
	...
	{
		MVector vUpperBoundTangent = curveData.controlPoints[3] - curveData.controlPoints[2];
	...
	else
	{
		...
		if (curveData.parameterization == 0)
			...
			currentParam = m_locator->getCurve().arcLengthToNaturalParameter(param, curveData.lengths);
		else
			// Calculate the natural parameter at the given normalized split-length
			currentParam = m_locator->splitLengthToNaturalParameter(param);
...
MPoint FlexiChainSingle_ScaleAdjustmentManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiChainSingle_ScaleAdjustmentManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainSingle_ScaleAdjustmentManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainSingle_ScaleAdjustmentManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainSingle_ScaleAdjustmentManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainSingle_ScaleAdjustmentManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiChainSingle_ScaleAdjustmentManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane,
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiChainSingle_ScaleAdjustmentManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiChainSingle_ScaleAdjustmentManip::updateOffsetHandle()
{
...
MStatus FlexiChainSingle_ScaleAdjustmentManip::updateOffsetHandlePlug(double offset)
{
	MStatus status;
...
FlexiChainSingle_ScaleAdjustmentManipContainer::FlexiChainSingle_ScaleAdjustmentManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiChainSingle_ScaleAdjustmentManipContainer::~FlexiChainSingle_ScaleAdjustmentManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiChainSingle_ScaleAdjustmentManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiChainSingle_ScaleAdjustmentManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiChainSingle_ScaleAdjustmentManip::kTypeName);
	MString manipName("FlexiChainSingleScaleAdjustmentManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiChainSingle_ScaleAdjustmentManip*)proxyManip;

	return status;
}

MStatus FlexiChainSingle_ScaleAdjustmentManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiChainSingle_ScaleAdjustmentContext::FlexiChainSingle_ScaleAdjustmentContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiChainSingle Scale Adjustment Tool");
	setTitleString(titleStr);
}

FlexiChainSingle_ScaleAdjustmentContext::~FlexiChainSingle_ScaleAdjustmentContext() {}
...
/*	Description
	...
	It instantiates manipulators for the currently hilited FlexiChainSingle nodes
...
void FlexiChainSingle_ScaleAdjustmentContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiChainSingle_ScaleAdjustmentContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiChainSingle_ScaleAdjustmentContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiChainSingle_ScaleAdjustmentContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiChainSingle_ScaleAdjustmentContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the offset for each scale adjustment falloff. Right click for more options.");
...
MStatus FlexiChainSingle_ScaleAdjustmentContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiChainSingle  :  Scale Adjustment Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiChainSingle  :  Scale Adjustment Tool  :  " + activeItems) + " Active Nodes";
...
/*	Description
	-----------
	This function is invoked by our callback which responds to RMB events on FlexiChainSingle nodes
...
MString FlexiChainSingle_ScaleAdjustmentContext::getHoverManipPlug()
{
	for (FlexiChainSingle_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
/*	Description
	-----------
	This function is invoked by the associated MPxSubSceneOverride registered to FlexiChainSingle nodes
...
MString FlexiChainSingle_ScaleAdjustmentContext::getActiveManipPlug()
{
	for (FlexiChainSingle_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiChainSingle_ScaleAdjustmentContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiChainSingle_ScaleAdjustmentContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiChainSingle_ScaleAdjustmentContext* context = (FlexiChainSingle_ScaleAdjustmentContext*)data;
	...
	// Create manipulators for each FlexiChainSingle object that is currently in hilite display mode and store them with the context so we can delete them later
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiChainSingle::kTypeId)
		{
			...
			const FlexiChainSingle::FlexiChainSingle_Data& curveData = locator->getCurveData();
			...
			{
				FlexiChainSingle_ScaleAdjustmentManip::physicalIndex = i;
				MString manipName(FlexiChainSingle_ScaleAdjustmentManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiChainSingle_ScaleAdjustmentManipContainer* manipContainer = (FlexiChainSingle_ScaleAdjustmentManipContainer*)FlexiChainSingle_ScaleAdjustmentManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiChainSingle_ScaleAdjustmentContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiChainSingle_ScaleAdjustmentContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	{
		if (plug == FlexiChainSingle::outputLocalFramesAttr || plug == FlexiChainSingle::outputLocalPositionsAttr ||
			plug == FlexiChainSingle::outputLocalOrientationsAttr || plug == FlexiChainSingle::outputWorldFramesAttr ||
			plug == FlexiChainSingle::outputWorldPositionsAttr || plug == FlexiChainSingle::outputWorldOrientationsAttr)
		{
			...
			FlexiChainSingle_ScaleAdjustmentContext* context = (FlexiChainSingle_ScaleAdjustmentContext*)data;
			for (FlexiChainSingle_ScaleAdjustmentManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiChainSingleScaleAdjustmentContext command is used to generate a custom context which can then be used to modify scale adjustments
	- A manipulator is created for each scaleAdjustment element plug, on each selected FlexiChainSingle node assigned to the active context
	- This command allows the user to query certain states relating to those manipulators

	MEL Command
	-----------
	FlexiChainSingleScaleAdjustmentContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiChainSingleScaleAdjustmentContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiChainSingle_ScaleAdjustmentContextCommand::FlexiChainSingle_ScaleAdjustmentContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiChainSingle_ScaleAdjustmentContextCommand::~FlexiChainSingle_ScaleAdjustmentContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiChainSingle_ScaleAdjustmentContextCommand::kHoverFlag = "-hmp";
const char* FlexiChainSingle_ScaleAdjustmentContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiChainSingle_ScaleAdjustmentContextCommand::kActiveFlag = "-amp";
const char* FlexiChainSingle_ScaleAdjustmentContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiChainSingle_ScaleAdjustmentContextCommand::makeObj()
{
	m_context = new FlexiChainSingle_ScaleAdjustmentContext;
	return m_context;
}

MStatus FlexiChainSingle_ScaleAdjustmentContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiChainSingle_ScaleAdjustmentContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiChainSingle_ScaleAdjustmentContextCommand::appendSyntax()
{
	MStatus status;
