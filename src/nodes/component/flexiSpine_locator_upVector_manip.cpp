...
/*	Description
	-----------
	The below setup is designed to provide a way to manually override the up-vector of a FlexiSpine node
	...
	Interface
	---------
	Plugin registration instantiates a single instance of the FlexiSpine_UpVectorContext (MPxSelectionContext) which will be named "FlexiSpineUpVectorContext1"
	A custom marking menu is also registered as a callback, such that right clicking a FlexiSpine node will bring up a menu allowing the user to activate the context
	Once activated, the context retrieves data from attributes on each FlexiSpine node that is registered to the active context
	Each node provides data to two manipulators, a start and end FlexiSpine_UpVectorManip (MPxManipulatorNode)
	Both manipulators are encapsulated by a FlexiSpine_UpVectorManipContainer (MPxManipContainer) because of a selection issue detailed below
...
FlexiSpine_UpVectorManip::FlexiSpine_UpVectorManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_directionIndex{ 0 },
	m_activeName{ 0 },
	m_handleName{ 0 },
...
FlexiSpine_UpVectorManip::~FlexiSpine_UpVectorManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiSpine_UpVectorManip::positionIndex = 0;
const MDistance FlexiSpine_UpVectorManip::m_handleRadius{ 1.5, MDistance::kCentimeters };

const float FlexiSpine_UpVectorManip::m_tickPoints[][4]
{
...
const float FlexiSpine_UpVectorManip::m_crossPoints[][4]
{
...
void FlexiSpine_UpVectorManip::postConstructor()
{
...
MStatus FlexiSpine_UpVectorManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiSpine_UpVectorManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiSpine_UpVectorManip::preDrawUI(const M3dView& view)
{
...
void FlexiSpine_UpVectorManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiSpine_UpVectorManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiSpineUpVectorContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiSpineUpVectorContext1";
...
MStatus FlexiSpine_UpVectorManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiSpine_UpVectorManip::doRelease(M3dView& view)
{
...
MStatus FlexiSpine_UpVectorManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiSpine_UpVectorManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;
	
	if (m_positionIndex == 0)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiSpine::normalUpVectorOverrideCompoundAttr };
	else if (m_positionIndex == 1)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiSpine::counterTwistUpVectorOverrideCompoundAttr };
...
MStatus FlexiSpine_UpVectorManip::getDirectionPlug(MPlug& directionPlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			directionPlug = compoundPlug.child(FlexiSpine::normalUpVectorOverrideAttr, &status);
		else if (m_positionIndex == 1)
			directionPlug = compoundPlug.child(FlexiSpine::counterTwistUpVectorOverrideAttr, &status);
	}
...
MStatus FlexiSpine_UpVectorManip::getStatePlug(MPlug& statePlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			statePlug = compoundPlug.child(FlexiSpine::normalUpVectorOverrideStateAttr, &status);
		else if (m_positionIndex == 1)
			statePlug = compoundPlug.child(FlexiSpine::counterTwistUpVectorOverrideStateAttr, &status);
	}
...
MString FlexiSpine_UpVectorManip::getCompoundPlugPartialName()
{
...
bool FlexiSpine_UpVectorManip::isEnabled()
{
	MStatus status;
...
bool FlexiSpine_UpVectorManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiSpine_UpVectorManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiSpine_UpVectorManip::isActive()
{
	return m_isActive;
}

// Legacy implementation
MPoint FlexiSpine_UpVectorManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiSpine_UpVectorManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiSpine_UpVectorManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiSpine_UpVectorManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiSpine_UpVectorManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiSpine_UpVectorManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiSpine_UpVectorManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane, 
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiSpine_UpVectorManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiSpine_UpVectorManip::updateManipState()
{
	m_isEnabled = isEnabled();
}

void FlexiSpine_UpVectorManip::updateManipPoints()
{
...
MStatus FlexiSpine_UpVectorManip::updatePlugDirection(MVector& vDirection)
{
	MStatus status;
...
FlexiSpine_UpVectorManipContainer::FlexiSpine_UpVectorManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiSpine_UpVectorManipContainer::~FlexiSpine_UpVectorManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiSpine_UpVectorManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiSpine_UpVectorManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiSpine_UpVectorManip::kTypeName);
	MString manipName("FlexiSpineManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiSpine_UpVectorManip*)proxyManip;

	return status;
}

MStatus FlexiSpine_UpVectorManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiSpine_UpVectorContext::FlexiSpine_UpVectorContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiSpine Up-Vector Tool");
	setTitleString(titleStr);
}

FlexiSpine_UpVectorContext::~FlexiSpine_UpVectorContext() {}
...
void FlexiSpine_UpVectorContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiSpine_UpVectorContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiSpine_UpVectorContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiSpine_UpVectorContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiSpine_UpVectorContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the up-vector to be used at each end. Right click for more options.");
...
MStatus FlexiSpine_UpVectorContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiSpine  :  Up-Vector Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiSpine  :  Up-Vector Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiSpine_UpVectorContext::getHoverManipPlug()
{
	for (FlexiSpine_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiSpine_UpVectorContext::getActiveManipPlug()
{
	for (FlexiSpine_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiSpine_UpVectorContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiSpine_UpVectorContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiSpine_UpVectorContext* context = (FlexiSpine_UpVectorContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiSpine::kTypeId)
		{
			...
			{
				FlexiSpine_UpVectorManip::positionIndex = i;
				MString manipName(FlexiSpine_UpVectorManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiSpine_UpVectorManipContainer* manipContainer = (FlexiSpine_UpVectorManipContainer*)FlexiSpine_UpVectorManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiSpine_UpVectorContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiSpine_UpVectorContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
			...
			plug == FlexiSpine::outputWorldPositionsAttr || plug == FlexiSpine::outputWorldOrientationsAttr)
		{
			...
			FlexiSpine_UpVectorContext* context = (FlexiSpine_UpVectorContext*)data;
			for (FlexiSpine_UpVectorManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiSpineUpVectorContext command is used to generate a custom context which can then be used to set manual overrides for the up-vectors
	...
	MEL Command
	-----------
	FlexiSpineUpVectorContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiSpineUpVectorContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiSpine_UpVectorContextCommand::FlexiSpine_UpVectorContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiSpine_UpVectorContextCommand::~FlexiSpine_UpVectorContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiSpine_UpVectorContextCommand::kHoverFlag = "-hmp";
const char* FlexiSpine_UpVectorContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiSpine_UpVectorContextCommand::kActiveFlag = "-amp";
const char* FlexiSpine_UpVectorContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiSpine_UpVectorContextCommand::makeObj()
{
	m_context = new FlexiSpine_UpVectorContext;
	return m_context;
}

MStatus FlexiSpine_UpVectorContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiSpine_UpVectorContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiSpine_UpVectorContextCommand::appendSyntax()
{
	MStatus status;
