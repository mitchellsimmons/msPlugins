...
/*	Description
	-----------
	The below setup is designed to provide a way to manually override the up-vector of a FlexiChainTriple node
	...
	Interface
	---------
	Plugin registration instantiates a single instance of the FlexiChainTriple_UpVectorContext (MPxSelectionContext) which will be named "FlexiChainTripleUpVectorContext1"
	A custom marking menu is also registered as a callback, such that right clicking a FlexiChainTriple node will bring up a menu allowing the user to activate the context
	Once activated, the context retrieves data from attributes on each FlexiChainTriple node that is registered to the active context
	Each node provides data to two manipulators, a start and end FlexiChainTriple_UpVectorManip (MPxManipulatorNode)
	Both manipulators are encapsulated by a FlexiChainTriple_UpVectorManipContainer (MPxManipContainer) because of a selection issue detailed below
...
FlexiChainTriple_UpVectorManip::FlexiChainTriple_UpVectorManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_directionIndex{ 0 },
	m_activeName{ 0 },
	m_handleName{ 0 },
...
FlexiChainTriple_UpVectorManip::~FlexiChainTriple_UpVectorManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiChainTriple_UpVectorManip::positionIndex = 0;
const MDistance FlexiChainTriple_UpVectorManip::m_handleRadius{ 1.5, MDistance::kCentimeters };

const float FlexiChainTriple_UpVectorManip::m_tickPoints[][4]
{
...
const float FlexiChainTriple_UpVectorManip::m_crossPoints[][4]
{
...
void FlexiChainTriple_UpVectorManip::postConstructor()
{
...
MStatus FlexiChainTriple_UpVectorManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiChainTriple_UpVectorManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiChainTriple_UpVectorManip::preDrawUI(const M3dView& view)
{
...
void FlexiChainTriple_UpVectorManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiChainTriple_UpVectorManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiChainTripleUpVectorContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiChainTripleUpVectorContext1";
...
MStatus FlexiChainTriple_UpVectorManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiChainTriple_UpVectorManip::doRelease(M3dView& view)
{
...
MStatus FlexiChainTriple_UpVectorManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiChainTriple_UpVectorManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;
	
	if (m_positionIndex == 0)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiChainTriple::normalUpVectorOverrideCompoundAttr };
	else if (m_positionIndex == 1)
		compoundPlug = MPlug{ m_locatorPath.node(), FlexiChainTriple::counterTwistUpVectorOverrideCompoundAttr };
...
MStatus FlexiChainTriple_UpVectorManip::getDirectionPlug(MPlug& directionPlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			directionPlug = compoundPlug.child(FlexiChainTriple::normalUpVectorOverrideAttr, &status);
		else if (m_positionIndex == 1)
			directionPlug = compoundPlug.child(FlexiChainTriple::counterTwistUpVectorOverrideAttr, &status);
	}
...
MStatus FlexiChainTriple_UpVectorManip::getStatePlug(MPlug& statePlug)
{
	MStatus status;
	...
	{
		if (m_positionIndex == 0)
			statePlug = compoundPlug.child(FlexiChainTriple::normalUpVectorOverrideStateAttr, &status);
		else if (m_positionIndex == 1)
			statePlug = compoundPlug.child(FlexiChainTriple::counterTwistUpVectorOverrideStateAttr, &status);
	}
...
MString FlexiChainTriple_UpVectorManip::getCompoundPlugPartialName()
{
...
bool FlexiChainTriple_UpVectorManip::isEnabled()
{
	MStatus status;
...
bool FlexiChainTriple_UpVectorManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiChainTriple_UpVectorManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiChainTriple_UpVectorManip::isActive()
{
	return m_isActive;
}

// Legacy implementation
MPoint FlexiChainTriple_UpVectorManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiChainTriple_UpVectorManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainTriple_UpVectorManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainTriple_UpVectorManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainTriple_UpVectorManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainTriple_UpVectorManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiChainTriple_UpVectorManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane, 
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiChainTriple_UpVectorManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiChainTriple_UpVectorManip::updateManipState()
{
	m_isEnabled = isEnabled();
}

void FlexiChainTriple_UpVectorManip::updateManipPoints()
{
...
MStatus FlexiChainTriple_UpVectorManip::updatePlugDirection(MVector& vDirection)
{
	MStatus status;
...
FlexiChainTriple_UpVectorManipContainer::FlexiChainTriple_UpVectorManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiChainTriple_UpVectorManipContainer::~FlexiChainTriple_UpVectorManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiChainTriple_UpVectorManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiChainTriple_UpVectorManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiChainTriple_UpVectorManip::kTypeName);
	MString manipName("FlexiChainTripleManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiChainTriple_UpVectorManip*)proxyManip;

	return status;
}

MStatus FlexiChainTriple_UpVectorManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiChainTriple_UpVectorContext::FlexiChainTriple_UpVectorContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiChainTriple Up-Vector Tool");
	setTitleString(titleStr);
}

FlexiChainTriple_UpVectorContext::~FlexiChainTriple_UpVectorContext() {}
...
void FlexiChainTriple_UpVectorContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiChainTriple_UpVectorContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiChainTriple_UpVectorContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiChainTriple_UpVectorContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiChainTriple_UpVectorContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the up-vector to be used at each end. Right click for more options.");
...
MStatus FlexiChainTriple_UpVectorContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiChainTriple  :  Up-Vector Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiChainTriple  :  Up-Vector Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiChainTriple_UpVectorContext::getHoverManipPlug()
{
	for (FlexiChainTriple_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiChainTriple_UpVectorContext::getActiveManipPlug()
{
	for (FlexiChainTriple_UpVectorManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiChainTriple_UpVectorContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiChainTriple_UpVectorContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiChainTriple_UpVectorContext* context = (FlexiChainTriple_UpVectorContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiChainTriple::kTypeId)
		{
			...
			{
				FlexiChainTriple_UpVectorManip::positionIndex = i;
				MString manipName(FlexiChainTriple_UpVectorManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiChainTriple_UpVectorManipContainer* manipContainer = (FlexiChainTriple_UpVectorManipContainer*)FlexiChainTriple_UpVectorManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiChainTriple_UpVectorContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiChainTriple_UpVectorContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
			...
			plug == FlexiChainTriple::outputWorldPositionsAttr || plug == FlexiChainTriple::outputWorldOrientationsAttr)
		{
			...
			FlexiChainTriple_UpVectorContext* context = (FlexiChainTriple_UpVectorContext*)data;
			for (FlexiChainTriple_UpVectorManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiChainTripleUpVectorContext command is used to generate a custom context which can then be used to set manual overrides for the up-vectors
	...
	MEL Command
	-----------
	FlexiChainTripleUpVectorContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiChainTripleUpVectorContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiChainTriple_UpVectorContextCommand::FlexiChainTriple_UpVectorContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiChainTriple_UpVectorContextCommand::~FlexiChainTriple_UpVectorContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiChainTriple_UpVectorContextCommand::kHoverFlag = "-hmp";
const char* FlexiChainTriple_UpVectorContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiChainTriple_UpVectorContextCommand::kActiveFlag = "-amp";
const char* FlexiChainTriple_UpVectorContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiChainTriple_UpVectorContextCommand::makeObj()
{
	m_context = new FlexiChainTriple_UpVectorContext;
	return m_context;
}

MStatus FlexiChainTriple_UpVectorContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiChainTriple_UpVectorContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiChainTriple_UpVectorContextCommand::appendSyntax()
{
	MStatus status;
