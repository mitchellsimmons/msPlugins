...
FlexiChainDouble_ScaleAdjustmentManip::FlexiChainDouble_ScaleAdjustmentManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_physicalIndex{ 0 },
...
FlexiChainDouble_ScaleAdjustmentManip::~FlexiChainDouble_ScaleAdjustmentManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiChainDouble_ScaleAdjustmentManip::physicalIndex = 0;
const MDistance FlexiChainDouble_ScaleAdjustmentManip::m_offsetHandleRadius{ 1.2, MDistance::kCentimeters };
const double FlexiChainDouble_ScaleAdjustmentManip::m_offsetHandleDragIncrement = 0.001;

// ------ MPxNode------

void FlexiChainDouble_ScaleAdjustmentManip::postConstructor()
{
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::connectToDependNode(const MObject& node)
{
	MStatus status;

	MDagPath::getAPathTo(node, m_locatorPath);
	MFnDependencyNode fnDep{ node };
	m_locator = (FlexiChainDouble*)fnDep.userNode();
...
void FlexiChainDouble_ScaleAdjustmentManip::preDrawUI(const M3dView& view)
{
	...
	updateOffsetHandle();
	const FlexiChainDouble::FlexiChainDouble_Data& curveData = m_locator->getCurveData();
...
void FlexiChainDouble_ScaleAdjustmentManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiChainDoubleScaleAdjustmentContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiChainDoubleScaleAdjustmentContext1";
			MGlobal::executeCommand(activePlugCmd);

			// Update the ramp UI if it exists and is visible (see MRS_FlexiChainDouble_scaleAdjustmentUIReplace.mel for implementation)
			...
			MString updateUICmd = "MRS_FlexiChainDouble_scaleAdjustmentUIReplace " + getCompoundPlugPartialName();
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::doRelease(M3dView& view)
{
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;

	MPlug compoundArrayPlug{ m_locatorPath.node(), FlexiChainDouble::scaleAdjustmentCompoundAttr };
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::getRampCompoundPlug(MPlug& rampPlug)
{
	MStatus status;
	...
	if (status)
		rampPlug = compoundPlug.child(FlexiChainDouble::scaleAdjustmentRampAttr, &status);
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::getScalePlug(MPlug& scalePlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		scalePlug = compoundPlug.child(FlexiChainDouble::scaleAdjustmentValueAttr, &status);
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::getOffsetPlug(MPlug& offsetPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		offsetPlug = compoundPlug.child(FlexiChainDouble::scaleAdjustmentOffsetAttr, &status);
...
MString FlexiChainDouble_ScaleAdjustmentManip::getCompoundPlugPartialName()
{
...
bool FlexiChainDouble_ScaleAdjustmentManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiChainDouble_ScaleAdjustmentManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiChainDouble_ScaleAdjustmentManip::isActive()
{
	return m_isActive;
}
...
MPoint FlexiChainDouble_ScaleAdjustmentManip::sampleCurve(double param)
{
	MPoint pPosition;
	...
	{
		MVector vUpperBoundTangent = curveData.controlPoints1[3] - curveData.controlPoints1[2];
	...
	else
	{
		...
		if (curveData.parameterization == 0)
			currentParam = param * 2.0;
		else if (curveData.parameterization == 1)
			// Calculate the natural parameter at the given normalized arc-length
			currentParam = m_locator->getCurve().arcLengthToNaturalParameter(param, curveData.lengths) * 2.0;
		else
			// Calculate the natural parameter at the given normalized split-length
			currentParam = m_locator->splitLengthToNaturalParameter(param * 2.0);
...
MPoint FlexiChainDouble_ScaleAdjustmentManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiChainDouble_ScaleAdjustmentManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainDouble_ScaleAdjustmentManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainDouble_ScaleAdjustmentManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiChainDouble_ScaleAdjustmentManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiChainDouble_ScaleAdjustmentManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiChainDouble_ScaleAdjustmentManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane,
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiChainDouble_ScaleAdjustmentManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiChainDouble_ScaleAdjustmentManip::updateOffsetHandle()
{
...
MStatus FlexiChainDouble_ScaleAdjustmentManip::updateOffsetHandlePlug(double offset)
{
	MStatus status;
...
FlexiChainDouble_ScaleAdjustmentManipContainer::FlexiChainDouble_ScaleAdjustmentManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiChainDouble_ScaleAdjustmentManipContainer::~FlexiChainDouble_ScaleAdjustmentManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiChainDouble_ScaleAdjustmentManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiChainDouble_ScaleAdjustmentManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiChainDouble_ScaleAdjustmentManip::kTypeName);
	MString manipName("FlexiChainDoubleScaleAdjustmentManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiChainDouble_ScaleAdjustmentManip*)proxyManip;

	return status;
}

MStatus FlexiChainDouble_ScaleAdjustmentManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiChainDouble_ScaleAdjustmentContext::FlexiChainDouble_ScaleAdjustmentContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiChainDouble Scale Adjustment Tool");
	setTitleString(titleStr);
}

FlexiChainDouble_ScaleAdjustmentContext::~FlexiChainDouble_ScaleAdjustmentContext() {}
...
/*	Description
	...
	It instantiates manipulators for the currently hilited FlexiChainDouble nodes
...
void FlexiChainDouble_ScaleAdjustmentContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiChainDouble_ScaleAdjustmentContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiChainDouble_ScaleAdjustmentContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiChainDouble_ScaleAdjustmentContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiChainDouble_ScaleAdjustmentContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the offset for each scale adjustment falloff. Right click for more options.");
...
MStatus FlexiChainDouble_ScaleAdjustmentContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiChainDouble  :  Scale Adjustment Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiChainDouble  :  Scale Adjustment Tool  :  " + activeItems) + " Active Nodes";
...
/*	Description
	-----------
	This function is invoked by our callback which responds to RMB events on FlexiChainDouble nodes
...
MString FlexiChainDouble_ScaleAdjustmentContext::getHoverManipPlug()
{
	for (FlexiChainDouble_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
/*	Description
	-----------
	This function is invoked by the associated MPxSubSceneOverride registered to FlexiChainDouble nodes
...
MString FlexiChainDouble_ScaleAdjustmentContext::getActiveManipPlug()
{
	for (FlexiChainDouble_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiChainDouble_ScaleAdjustmentContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiChainDouble_ScaleAdjustmentContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiChainDouble_ScaleAdjustmentContext* context = (FlexiChainDouble_ScaleAdjustmentContext*)data;
	...
	// Create manipulators for each FlexiChainDouble object that is currently in hilite display mode and store them with the context so we can delete them later
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiChainDouble::kTypeId)
		{
			...
			const FlexiChainDouble::FlexiChainDouble_Data& curveData = locator->getCurveData();
			...
			{
				FlexiChainDouble_ScaleAdjustmentManip::physicalIndex = i;
				MString manipName(FlexiChainDouble_ScaleAdjustmentManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiChainDouble_ScaleAdjustmentManipContainer* manipContainer = (FlexiChainDouble_ScaleAdjustmentManipContainer*)FlexiChainDouble_ScaleAdjustmentManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiChainDouble_ScaleAdjustmentContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiChainDouble_ScaleAdjustmentContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	{
		if (plug == FlexiChainDouble::outputLocalFramesAttr || plug == FlexiChainDouble::outputLocalPositionsAttr ||
			plug == FlexiChainDouble::outputLocalOrientationsAttr || plug == FlexiChainDouble::outputWorldFramesAttr ||
			plug == FlexiChainDouble::outputWorldPositionsAttr || plug == FlexiChainDouble::outputWorldOrientationsAttr)
		{
			...
			FlexiChainDouble_ScaleAdjustmentContext* context = (FlexiChainDouble_ScaleAdjustmentContext*)data;
			for (FlexiChainDouble_ScaleAdjustmentManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiChainDoubleScaleAdjustmentContext command is used to generate a custom context which can then be used to modify scale adjustments
	- A manipulator is created for each scaleAdjustment element plug, on each selected FlexiChainDouble node assigned to the active context
	- This command allows the user to query certain states relating to those manipulators

	MEL Command
	-----------
	FlexiChainDoubleScaleAdjustmentContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiChainDoubleScaleAdjustmentContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiChainDouble_ScaleAdjustmentContextCommand::FlexiChainDouble_ScaleAdjustmentContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiChainDouble_ScaleAdjustmentContextCommand::~FlexiChainDouble_ScaleAdjustmentContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiChainDouble_ScaleAdjustmentContextCommand::kHoverFlag = "-hmp";
const char* FlexiChainDouble_ScaleAdjustmentContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiChainDouble_ScaleAdjustmentContextCommand::kActiveFlag = "-amp";
const char* FlexiChainDouble_ScaleAdjustmentContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiChainDouble_ScaleAdjustmentContextCommand::makeObj()
{
	m_context = new FlexiChainDouble_ScaleAdjustmentContext;
	return m_context;
}

MStatus FlexiChainDouble_ScaleAdjustmentContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiChainDouble_ScaleAdjustmentContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiChainDouble_ScaleAdjustmentContextCommand::appendSyntax()
{
	MStatus status;
