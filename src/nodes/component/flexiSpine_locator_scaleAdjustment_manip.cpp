...
FlexiSpine_ScaleAdjustmentManip::FlexiSpine_ScaleAdjustmentManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_physicalIndex{ 0 },
...
FlexiSpine_ScaleAdjustmentManip::~FlexiSpine_ScaleAdjustmentManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiSpine_ScaleAdjustmentManip::physicalIndex = 0;
const MDistance FlexiSpine_ScaleAdjustmentManip::m_offsetHandleRadius{ 1.2, MDistance::kCentimeters };
const double FlexiSpine_ScaleAdjustmentManip::m_offsetHandleDragIncrement = 0.001;

// ------ MPxNode------

void FlexiSpine_ScaleAdjustmentManip::postConstructor()
{
...
MStatus FlexiSpine_ScaleAdjustmentManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiSpine_ScaleAdjustmentManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiSpine_ScaleAdjustmentManip::preDrawUI(const M3dView& view)
{
...
void FlexiSpine_ScaleAdjustmentManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiSpine_ScaleAdjustmentManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiSpineScaleAdjustmentContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiSpineScaleAdjustmentContext1";
			MGlobal::executeCommand(activePlugCmd);

			// Update the ramp UI if it exists and is visible (see MRS_FlexiSpine_scaleAdjustmentUIReplace.mel for implementation)
			...
			MString updateUICmd = "MRS_FlexiSpine_scaleAdjustmentUIReplace " + getCompoundPlugPartialName();
...
MStatus FlexiSpine_ScaleAdjustmentManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiSpine_ScaleAdjustmentManip::doRelease(M3dView& view)
{
...
MStatus FlexiSpine_ScaleAdjustmentManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiSpine_ScaleAdjustmentManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;

	MPlug compoundArrayPlug{ m_locatorPath.node(), FlexiSpine::scaleAdjustmentCompoundAttr };
...
MStatus FlexiSpine_ScaleAdjustmentManip::getRampCompoundPlug(MPlug& rampPlug)
{
	MStatus status;
	...
	if (status)
		rampPlug = compoundPlug.child(FlexiSpine::scaleAdjustmentRampAttr, &status);
...
MStatus FlexiSpine_ScaleAdjustmentManip::getScalePlug(MPlug& scalePlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		scalePlug = compoundPlug.child(FlexiSpine::scaleAdjustmentValueAttr, &status);
...
MStatus FlexiSpine_ScaleAdjustmentManip::getOffsetPlug(MPlug& offsetPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		offsetPlug = compoundPlug.child(FlexiSpine::scaleAdjustmentOffsetAttr, &status);
...
MString FlexiSpine_ScaleAdjustmentManip::getCompoundPlugPartialName()
{
...
bool FlexiSpine_ScaleAdjustmentManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiSpine_ScaleAdjustmentManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiSpine_ScaleAdjustmentManip::isActive()
{
	return m_isActive;
}
...
MPoint FlexiSpine_ScaleAdjustmentManip::sampleCurve(double param)
{
	MPoint pPosition;
...
MPoint FlexiSpine_ScaleAdjustmentManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiSpine_ScaleAdjustmentManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiSpine_ScaleAdjustmentManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiSpine_ScaleAdjustmentManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiSpine_ScaleAdjustmentManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiSpine_ScaleAdjustmentManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiSpine_ScaleAdjustmentManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane,
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiSpine_ScaleAdjustmentManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiSpine_ScaleAdjustmentManip::updateOffsetHandle()
{
...
MStatus FlexiSpine_ScaleAdjustmentManip::updateOffsetHandlePlug(double offset)
{
	MStatus status;
...
FlexiSpine_ScaleAdjustmentManipContainer::FlexiSpine_ScaleAdjustmentManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiSpine_ScaleAdjustmentManipContainer::~FlexiSpine_ScaleAdjustmentManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiSpine_ScaleAdjustmentManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiSpine_ScaleAdjustmentManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiSpine_ScaleAdjustmentManip::kTypeName);
	MString manipName("FlexiSpineScaleAdjustmentManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiSpine_ScaleAdjustmentManip*)proxyManip;

	return status;
}

MStatus FlexiSpine_ScaleAdjustmentManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiSpine_ScaleAdjustmentContext::FlexiSpine_ScaleAdjustmentContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiSpine Scale Adjustment Tool");
	setTitleString(titleStr);
}

FlexiSpine_ScaleAdjustmentContext::~FlexiSpine_ScaleAdjustmentContext() {}
...
void FlexiSpine_ScaleAdjustmentContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	if (!status)
		std::cerr << "FlexiSpine_ScaleAdjustmentContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiSpine_ScaleAdjustmentContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiSpine_ScaleAdjustmentContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiSpine_ScaleAdjustmentContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the offset for each scale adjustment falloff. Right click for more options.");
...
MStatus FlexiSpine_ScaleAdjustmentContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiSpine  :  Scale Adjustment Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiSpine  :  Scale Adjustment Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiSpine_ScaleAdjustmentContext::getHoverManipPlug()
{
	for (FlexiSpine_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiSpine_ScaleAdjustmentContext::getActiveManipPlug()
{
	for (FlexiSpine_ScaleAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiSpine_ScaleAdjustmentContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiSpine_ScaleAdjustmentContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiSpine_ScaleAdjustmentContext* context = (FlexiSpine_ScaleAdjustmentContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiSpine::kTypeId)
		{
			...
			{
				FlexiSpine_ScaleAdjustmentManip::physicalIndex = i;
				MString manipName(FlexiSpine_ScaleAdjustmentManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiSpine_ScaleAdjustmentManipContainer* manipContainer = (FlexiSpine_ScaleAdjustmentManipContainer*)FlexiSpine_ScaleAdjustmentManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiSpine_ScaleAdjustmentContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiSpine_ScaleAdjustmentContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
			...
			plug == FlexiSpine::outputWorldPositionsAttr || plug == FlexiSpine::outputWorldOrientationsAttr)
		{
			...
			FlexiSpine_ScaleAdjustmentContext* context = (FlexiSpine_ScaleAdjustmentContext*)data;
			for (FlexiSpine_ScaleAdjustmentManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiSpineScaleAdjustmentContext command is used to generate a custom context which can then be used to modify scale adjustments
	- A manipulator is created for each scaleAdjustment element plug, on each selected FlexiSpine node assigned to the active context
	- This command allows the user to query certain states relating to those manipulators

	MEL Command
	-----------
	FlexiSpineScaleAdjustmentContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiSpineScaleAdjustmentContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiSpine_ScaleAdjustmentContextCommand::FlexiSpine_ScaleAdjustmentContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiSpine_ScaleAdjustmentContextCommand::~FlexiSpine_ScaleAdjustmentContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiSpine_ScaleAdjustmentContextCommand::kHoverFlag = "-hmp";
const char* FlexiSpine_ScaleAdjustmentContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiSpine_ScaleAdjustmentContextCommand::kActiveFlag = "-amp";
const char* FlexiSpine_ScaleAdjustmentContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiSpine_ScaleAdjustmentContextCommand::makeObj()
{
	m_context = new FlexiSpine_ScaleAdjustmentContext;
	return m_context;
}

MStatus FlexiSpine_ScaleAdjustmentContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiSpine_ScaleAdjustmentContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiSpine_ScaleAdjustmentContextCommand::appendSyntax()
{
	MStatus status;
