...
FlexiInstancer_PositionAdjustmentManip::FlexiInstancer_PositionAdjustmentManip() : MPxManipulatorNode(),
	m_locator{ nullptr },
	m_physicalIndex{ 0 },
...
FlexiInstancer_PositionAdjustmentManip::~FlexiInstancer_PositionAdjustmentManip()
{
	m_locator = nullptr;
}

// Static
unsigned int FlexiInstancer_PositionAdjustmentManip::physicalIndex = 0;
const MDistance FlexiInstancer_PositionAdjustmentManip::m_offsetHandleRadius{ 1.2, MDistance::kCentimeters };
const double FlexiInstancer_PositionAdjustmentManip::m_offsetHandleDragIncrement = 0.001;

// ------ MPxNode------

void FlexiInstancer_PositionAdjustmentManip::postConstructor()
{
...
MStatus FlexiInstancer_PositionAdjustmentManip::initialize()
{
	// No-op
	return MS::kSuccess;
}
...
MStatus FlexiInstancer_PositionAdjustmentManip::connectToDependNode(const MObject& node)
{
	MStatus status;
...
void FlexiInstancer_PositionAdjustmentManip::preDrawUI(const M3dView& view)
{
	...
	m_pStart = curveData.vLowerBoundPoint;
	m_pStart *= m_locatorPath.inclusiveMatrix();
	m_pEnd = curveData.vUpperBoundPoint;
...
void FlexiInstancer_PositionAdjustmentManip::drawUI(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext) const
{
...
MStatus FlexiInstancer_PositionAdjustmentManip::doPress(M3dView& view)
{
	if (glActiveName(m_activeName))
	{
		if (m_activeName == m_handleName)
		{
			...
			MString activePlugCmd = "FlexiInstancerPositionAdjustmentContext -e -activeManipPlug " + getCompoundPlugPartialName();
			activePlugCmd += " FlexiInstancerPositionAdjustmentContext1";
			MGlobal::executeCommand(activePlugCmd);

			// Update the ramp UI if it exists and is visible (see MRS_FlexiInstancer_positionAdjustmentUIReplace.mel for implementation)
			...
			MString updateUICmd = "MRS_FlexiInstancer_positionAdjustmentUIReplace " + getCompoundPlugPartialName();
...
MStatus FlexiInstancer_PositionAdjustmentManip::doDrag(M3dView& view)
{
	MStatus status;
...
MStatus FlexiInstancer_PositionAdjustmentManip::doRelease(M3dView& view)
{
...
MStatus FlexiInstancer_PositionAdjustmentManip::doMove(M3dView& view, bool& refresh)
{
	MPoint pCameraWorld = getCameraPosition(view);
...
MStatus FlexiInstancer_PositionAdjustmentManip::getCompoundPlug(MPlug& compoundPlug)
{
	MStatus status;

	MPlug compoundArrayPlug{ m_locatorPath.node(), FlexiInstancer::positionAdjustmentCompoundAttr };
...
MStatus FlexiInstancer_PositionAdjustmentManip::getRampCompoundPlug(MPlug& rampPlug)
{
	MStatus status;
	...
	if (status)
		rampPlug = compoundPlug.child(FlexiInstancer::positionAdjustmentRampAttr, &status);
...
/*	Description
	-----------
	Function retrieves the position plug associated with this manipulator    */
MStatus FlexiInstancer_PositionAdjustmentManip::getPositionPlug(MPlug& positionPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		positionPlug = compoundPlug.child(FlexiInstancer::positionAdjustmentValueAttr, &status);
...
MStatus FlexiInstancer_PositionAdjustmentManip::getOffsetPlug(MPlug& offsetPlug)
{
	MStatus status;
	...
	status = getCompoundPlug(compoundPlug);
	if (status)
		offsetPlug = compoundPlug.child(FlexiInstancer::positionAdjustmentOffsetAttr, &status);
...
MString FlexiInstancer_PositionAdjustmentManip::getCompoundPlugPartialName()
{
...
bool FlexiInstancer_PositionAdjustmentManip::isUserInteracting()
{
	return m_isUserInteracting;
}

bool FlexiInstancer_PositionAdjustmentManip::isMouseHovered()
{
	return m_isMouseHovered;
}

bool FlexiInstancer_PositionAdjustmentManip::isActive()
{
	return m_isActive;
}
...
MPoint FlexiInstancer_PositionAdjustmentManip::sampleCurve(double param)
{
	MPoint pPosition;
	...
	if (param < 0.0)
	{
		...
		pPosition = (curveData.vLowerBoundPoint + param * arcLength * vLowerBoundTangent) * m_locatorPath.inclusiveMatrix();
	}
	else if (param > 1.0)
	{
		MVector vUpperBoundTangent = m_locator->sampleFirstDerivative(curveData.upperBoundKnot);
		vUpperBoundTangent.normalize();
		pPosition = (curveData.vUpperBoundPoint + (param - 1.0) * arcLength * vUpperBoundTangent) * m_locatorPath.inclusiveMatrix();
	}
	else
	{
...
MPoint FlexiInstancer_PositionAdjustmentManip::getCameraPosition(M3dView& view) const
{
	MDagPath cameraPath;
...
// VP2 implementation
MPoint FlexiInstancer_PositionAdjustmentManip::getCameraPosition(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiInstancer_PositionAdjustmentManip::getCameraAimDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiInstancer_PositionAdjustmentManip::getCameraAimDirection(const MHWRender::MFrameContext& frameContext) const
{
...
// Legacy implementation
MVector FlexiInstancer_PositionAdjustmentManip::getCameraUpDirection(M3dView& view) const
{
	MDagPath cameraPath;
	view.getCamera(cameraPath);
	cameraPath.pop();
...
// VP2 implementation
MVector FlexiInstancer_PositionAdjustmentManip::getCameraUpDirection(const MHWRender::MFrameContext& frameContext) const
{
...
bool FlexiInstancer_PositionAdjustmentManip::computeMouseIntersection(const MPoint& pMouse, const MVector& vMouseDirection, const MPoint& pPointOnInterPlane,
	const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
	...
	// Position the mouse direction vector by the ratio then add to the mouse position to get the intersection point
...
/*	Description
	...
	Returns true if the raycast produces a successful intersection    */
bool FlexiInstancer_PositionAdjustmentManip::computeCurrentMouseIntersection(const MPoint& pPointOnInterPlane, const MVector& vInterPlaneNormal, MPoint& pIntersection)
{
...
void FlexiInstancer_PositionAdjustmentManip::updateOffsetHandle()
{
...
MStatus FlexiInstancer_PositionAdjustmentManip::updateOffsetHandlePlug(double offset)
{
	MStatus status;
...
FlexiInstancer_PositionAdjustmentManipContainer::FlexiInstancer_PositionAdjustmentManipContainer() : MPxManipContainer(),
	m_manip{ nullptr },
	m_locator{ nullptr }
{}

FlexiInstancer_PositionAdjustmentManipContainer::~FlexiInstancer_PositionAdjustmentManipContainer()
{
	m_manip = nullptr;
	m_locator = nullptr;
}
...
MStatus FlexiInstancer_PositionAdjustmentManipContainer::initialize()
{
	return MPxManipContainer::initialize();
}

MStatus FlexiInstancer_PositionAdjustmentManipContainer::createChildren()
{
	MStatus status;
	
	MPxManipulatorNode *proxyManip = 0;
	MString manipTypeName(FlexiInstancer_PositionAdjustmentManip::kTypeName);
	MString manipName("FlexiInstancerPositionAdjustmentManipInst");
	status = addMPxManipulatorNode(manipTypeName, manipName, proxyManip);

	if (status)
		m_manip = (FlexiInstancer_PositionAdjustmentManip*)proxyManip;

	return status;
}

MStatus FlexiInstancer_PositionAdjustmentManipContainer::connectToDependNode(const MObject& node)
{
	MStatus status;
...
FlexiInstancer_PositionAdjustmentContext::FlexiInstancer_PositionAdjustmentContext() : MPxSelectionContext(),
	m_numActiveNodes{ 0 },
	m_selectionChangedCallbackId{ 0 }
{
	MString titleStr("FlexiInstancer Position Adjustment Tool");
	setTitleString(titleStr);
}

FlexiInstancer_PositionAdjustmentContext::~FlexiInstancer_PositionAdjustmentContext() {}
...
void FlexiInstancer_PositionAdjustmentContext::toolOnSetup(MEvent& event)
{
	MStatus status;
	...
	MString helpStr("Drag manipulators to define the offset for each position adjustment falloff. Right click for more options.");
	...
	if (!status)
		std::cerr << "FlexiInstancer_PositionAdjustmentContext: MEventMessage::addEventCallback() failed" << std::endl;
...
void FlexiInstancer_PositionAdjustmentContext::toolOffCleanup()
{
	MStatus status;
	...
	if (!status)
		cerr << "FlexiInstancer_PositionAdjustmentContext: MNodeMessage::removeCallback() failed" << std::endl;
	m_selectionChangedCallbackId = 0;
...
MStatus FlexiInstancer_PositionAdjustmentContext::doEnterRegion(MEvent& event)
{
	MString helpStr("Drag manipulators to define the offset for each position adjustment falloff. Right click for more options.");
...
MStatus FlexiInstancer_PositionAdjustmentContext::drawFeedback(MHWRender::MUIDrawManager& drawManager, const MHWRender::MFrameContext& frameContext)
{
	...
	if (m_numActiveNodes == 1)
		text = ("FlexiInstancer  :  Position Adjustment Tool  :  " + activeItems) + " Active Node";
	else
		text = ("FlexiInstancer  :  Position Adjustment Tool  :  " + activeItems) + " Active Nodes";
...
MString FlexiInstancer_PositionAdjustmentContext::getHoverManipPlug()
{
	for (FlexiInstancer_PositionAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
MString FlexiInstancer_PositionAdjustmentContext::getActiveManipPlug()
{
	for (FlexiInstancer_PositionAdjustmentManipContainer*& manipContainer : m_manipContainers)
	{
...
void FlexiInstancer_PositionAdjustmentContext::setActiveManipPlug(MString& activeManipPlugName)
{
	MStatus status;
...
void FlexiInstancer_PositionAdjustmentContext::updateManipulatorsCallback(void* data)
{
	MStatus status;

	FlexiInstancer_PositionAdjustmentContext* context = (FlexiInstancer_PositionAdjustmentContext*)data;
	...
	while (!iter.isDone())
	{
		...
		if (!seen && fnDep.typeId() == FlexiInstancer::kTypeId)
		{
			...
			const FlexiInstancer::FlexiInstancer_Data& curveData = locator->getCurveData();
			if (curveData.isPositionAdjustmentEnabled)
				locator->computePositionAdjustments(dataBlock);
			...
			MPlug positionAdjustmentPlug = fnDep.findPlug("positionAdjustment", true, &status);
			for (unsigned int i = 0; i < positionAdjustmentPlug.evaluateNumElements(); ++i)
			{
				FlexiInstancer_PositionAdjustmentManip::physicalIndex = i;
				MString manipName(FlexiInstancer_PositionAdjustmentManipContainer::kTypeName);
				MObject manipObject = MObject::kNullObj;
				FlexiInstancer_PositionAdjustmentManipContainer* manipContainer = (FlexiInstancer_PositionAdjustmentManipContainer*)FlexiInstancer_PositionAdjustmentManipContainer::newManipulator(manipName, manipObject);
			...
			if (status)
				...
				std::cerr << "FlexiInstancer_PositionAdjustmentContext: MNodeMessage::addAttributeChangedCallback() failed" << std::endl;
		}
...
void FlexiInstancer_PositionAdjustmentContext::nodePlugChangedCallback(MNodeMessage::AttributeMessage msg, MPlug& plug, MPlug& otherPlug, void* data)
{
	...
	if (msg & MNodeMessage::AttributeMessage::kAttributeEval)
	{
		if (plug == FlexiInstancer::outputLocalParticleArrayAttr || plug == FlexiInstancer::outputWorldParticleArrayAttr)
		{
			...
			FlexiInstancer_PositionAdjustmentContext* context = (FlexiInstancer_PositionAdjustmentContext*)data;
			for (FlexiInstancer_PositionAdjustmentManipContainer*& manipContainer : context->m_manipContainers)
			{
...
/*	Description
	-----------
	The FlexiInstancerPositionAdjustmentContext command is used to generate a custom context which can then be used to modify position adjustments
	- A manipulator is created for each positionAdjustment element plug, on each selected FlexiInstancer node assigned to the active context
	- This command allows the user to query certain states relating to those manipulators

	MEL Command
	-----------
	FlexiInstancerPositionAdjustmentContext [-hoverManipPlug bool] [-activeManipPlug bool] [context]
	...
	----
	context
		This is the name given to the FlexiInstancerPositionAdjustmentContext instance upon creation
		When querying the active context, this name must be passed to the command
*/

FlexiInstancer_PositionAdjustmentContextCommand::FlexiInstancer_PositionAdjustmentContextCommand() : MPxContextCommand(),
	m_context{ nullptr }
{}

FlexiInstancer_PositionAdjustmentContextCommand::~FlexiInstancer_PositionAdjustmentContextCommand()
{
	m_context = nullptr;
}
...
const char* FlexiInstancer_PositionAdjustmentContextCommand::kHoverFlag = "-hmp";
const char* FlexiInstancer_PositionAdjustmentContextCommand::kHoverFlagLong = "-hoverManipPlug";
const char* FlexiInstancer_PositionAdjustmentContextCommand::kActiveFlag = "-amp";
const char* FlexiInstancer_PositionAdjustmentContextCommand::kActiveFlagLong = "-activeManipPlug";

MPxContext* FlexiInstancer_PositionAdjustmentContextCommand::makeObj()
{
	m_context = new FlexiInstancer_PositionAdjustmentContext;
	return m_context;
}

MStatus FlexiInstancer_PositionAdjustmentContextCommand::doEditFlags()
{
	MStatus status;
...
MStatus FlexiInstancer_PositionAdjustmentContextCommand::doQueryFlags()
{
	MStatus status;
	MArgParser argParser = parser();
...
MStatus FlexiInstancer_PositionAdjustmentContextCommand::appendSyntax()
{
	MStatus status;
